【メニューSE追加プラグイン Ver2.00】
http://hororo.wp.xdomain.jp/198/

2021/05/30更新 v507b対応版
...
　にSE（効果音）を指定するプラグインです。
	...
	button 等に name を指定する事で、同じnameのボタンにSEを指定できます。
　　
　◆出来ない事
...
　
　③ first.ks に下記コードを記述し、プラグインを呼び出してください。
　　[plugin name=menu_se]
　　
...
　　　　cancel              //ダイアログのcancel
　　　　※任意のname　　　　 //buttonタグなどのnameを指定できます。
...
　　① このプラグインのinit.ks から [sysview] を削除してください。（menu_seの中のhtmlファイルも削除してOK）
　　② 併用するプラグインで使用するhtmlの一番下か上に、下記コードを追記してください。
